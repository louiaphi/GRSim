...
public class SimSingLight : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        //setup rendering
        CamMaterial.SetTexture("_MainTex", Picture);
    ...
    public bool HitBlackHole(Vector4 newPos)
    {
        ...
        if (CalcR(newPos, a) <= EventHorizonRadius * SuckInMargin) // sometimes flicks ray away if too close to event horizon
        {
            return true;
        }
        return false;
    ...
    void Update()
    {
        //if (Render)
        //{
        //    Texture2D Picture = RenderTexture();
        //    CamMaterial.SetTexture("_MainTex", Picture);
        //    DrawTexture(Picture, CamMaterial, MonitorSize.x, MonitorSize.y);
        //    Render = false;
        //}
        if (Render && RenderStep == -1) // start rendering if not already
        {
            RenderStep = 0;
            x = 0; // start at first pixel;
            y = 0;
            Render = false; 
            startT = Time.time;
        }
        if (RenderStep > MonitorSize.x * MonitorSize.y)
        {
            RenderStep = -1; // stop rendering
        }
        if (RenderStep > -1)
        {
            if (x >= MonitorSize.x)
            ...
            RenderStep++;
            Debug.Log("Step: " + RenderStep);
            Debug.Log("= " + (100 * RenderStep / MonitorSize.x / MonitorSize.y) + "%");
            Debug.Log("Time = " + (Time.time - startT) + "s");
            Debug.Log("Expected Time left:" + ((Time.time - startT) / (RenderStep / MonitorSize.x / MonitorSize.y) - Time.time) + "s");
            x++;
        }
    }
}
