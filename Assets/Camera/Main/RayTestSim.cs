...
public class RayTestSim : MonoBehaviour
{
    ...
    public float preciseSteps;
    public float StepSizePow;
    ...
    void Start()
    {
        ...
        InvMetricTensor = Matrix4x4.identity;
    ...
    public void StepLightNTimes(LightRay ray, int n, Color c)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            float precision = StepSize;// * Mathf.Pow(r - 2 * M, StepSizePow);
            if (r - 2 * M < margin)
            {
            ...
            Debug.LogWarning("gkk: " + gkk(nextPos.k));
    ...
    public void FindAsymetries()
    {
        ...
        Vector4 posy = new Vector4(0, 0, 10, 0);
        ...
        Debug.LogError("Metric Tensor");
        Debug.LogError("x:");
        CalcMetricTensor(posx, CalcR(posx, a), a, M);
        LogTensor(MetricTensor);
        Debug.LogError("y:");
        CalcMetricTensor(posy, CalcR(posy, a), a, M);
        LogTensor(MetricTensor);
        Debug.LogError("DMetric:");
        ...
        Debug.LogError("m2:");
        LogTensorArray(m2);
        Debug.LogError("Dif DelMetric Tensor");
        LogTensorArray(
            matrix4X4ArraySub(
                m1,
                m2));
        Debug.LogError("Dif Mag DMetric Tensor");
        float mm1 = TensorArrayMagnitude(m1);
        float mm2 = TensorArrayMagnitude(m2);
        Debug.Log(mm1 - mm2);
        CalcMetricTensor(CameraPosition, CalcR(CameraPosition, a), a, M); //Calculate Metric Tensor at Camera Position
        MetricTensorAtCam = MetricTensor;
        localTetradAtCam = localTetrad(posx);
        localTetradAtCam = rotateLocalTetrad(localTetradAtCam, CameraRotation);
        LightRay Ray = instantiateRay(CameraPosition, FOV, 11, 11, new Vector2(5, 10), localTetradAtCam); //put together later
        Color colour = new Color(1, 0, 0);
        StepLightNTimes(Ray, N, colour);
        Ray = instantiateRay(CameraPosition, FOV, 11, 11, new Vector2(10, 5), localTetradAtCam); //put together later
    ...
    {
        Matrix4x4 Rtest = Matrix4x4.Rotate(Quaternion.Euler(Angles));  // :skull: ... why did i listen at first to ChatGPT and deactivate my brain and write down all the rotation matrices myself
    ...
    public Matrix4x4 RotateMatrix4x4(Matrix4x4 matrix, Vector3 angles) //dont know if this will work: trying to rotate the basis vectors before Gram-Schmidt
    {
        Vector4 c0 = matrix.GetColumn(0);
        Vector4 c1 = matrix.GetColumn(1);
        Vector4 c2 = matrix.GetColumn(2);
        Vector4 c3 = matrix.GetColumn(3);
        Matrix4x4 RotationMatrix = Matrix4x4.Rotate(Quaternion.Euler(angles));
        c0 = RotationMatrix * c0;
        c1 = RotationMatrix * c1;
        c2 = RotationMatrix * c2;
        c3 = RotationMatrix * c3;
        Matrix4x4 output = new Matrix4x4(c0, c1, c2, c3);
        return output;
    }

    public Matrix4x4 CameraTetradV2() //v2 and v3 useless because v1 was already correct, so ca. 15h wasted.........................................
    {
        Vector3 forward3D = Matrix4x4.Rotate(Quaternion.Euler(CameraRotation.x, CameraRotation.y, CameraRotation.z)) * new Vector3(1, 0, 0);
        Vector4 forward = new Vector4(0, forward3D.x, forward3D.y, forward3D.z);
        Vector3 right3D = Vector3.Cross(Vector3.up, forward3D);
        Vector4 right = new Vector4(0, right3D.x, right3D.y, right3D.z);
        Vector3 up3D = Vector3.Cross(right3D, forward3D);
        Vector4 up = new Vector4(0, up3D.x, up3D.y, up3D.z);
        forward.Normalize();
        right.Normalize();
        up.Normalize();
        ...
        Matrix4x4 preE = new Matrix4x4(
            new Vector4(1 / Mathf.Sqrt(Mathf.Abs(MetricTensorAtCam[0, 0])), 0, 0, 0),
            right,
            up,
            forward
            );
        Matrix4x4 e = Matrix4x4.zero;
        e.SetColumn(0, preE.GetColumn(0));
        for ( int i = 1; i < 4; i++ )
        {
            float divid = 0;
            float divis = 0;
            for (int alpha = 0; alpha < 4; alpha++)
            {
                for (int beta = 0; beta < 4; beta++)
                {
                    divid += MetricTensorAtCam[alpha, beta] * preE[alpha, i] * preE[beta, 0];
                }
            }
            for (int delta = 0; delta < 4; delta++)
            {
                ...
                {
                    divis += MetricTensorAtCam[delta, gamma] * preE[gamma, 0] * preE[delta, 0];
                }
            }
            if (Mathf.Abs(divis) < 0.001f)
            {
                Debug.LogError("Alarm!");
            }
            for ( int mu = 0; mu < 4; mu++ ) 
            {

                e[mu, i] = preE[mu, i] - divid / divis * preE[mu, 0];
            }
        }
        Matrix4x4 normE = Matrix4x4.zero;
        normE.SetColumn(0, e.GetColumn(0));
        for (int i = 1; i < 4; ++i)
        {
            float divis = 0;
            for (int alpha = 0; alpha < 4; alpha++)
            {
                for (int beta = 0; beta < 4; beta++)
                {
                    divis += MetricTensorAtCam[alpha, beta] * e[alpha, i] * e[beta, i];
                }
            }
            divis = Mathf.Sqrt(Mathf.Abs(divis));
            if (Mathf.Abs(divis) < 0.001f)
            {
                Debug.LogError("Alarm!");
            }
            for (int mu = 0; mu < 4; mu++)
            {
                normE[mu, i] = e[mu, i] / divis;
            }
        }
        return normE;
    }

    public Matrix4x4 CameraTetradV3()
    {
        Vector3 forward3D = Matrix4x4.Rotate(Quaternion.Euler(CameraRotation.x, CameraRotation.y, CameraRotation.z)) * new Vector3(0, 0, 1);
        Vector4 forward = new Vector4(0, forward3D.x, forward3D.y, forward3D.z);
        Vector3 right3D = Vector3.Cross(forward3D, Vector3.up);
        Vector4 right = new Vector4(0, right3D.x, right3D.y, right3D.z);
        Vector3 up3D = Vector3.Cross(right3D, forward3D);
        Vector4 up = new Vector4(0, up3D.x, up3D.y, up3D.z);
        forward.Normalize();
